...
from node_clustering_algoritms import node_clustering
...
from linear_gae.optimizer import *
...
# Clustern algorithm
flags.DEFINE_string('clustering_algorithm','kmeans','Algorithm for node clustering \
...
for i in range(FLAGS.nb_run):
    ...
    # Node Clustering: K-Means clustering in embedding space
        ...
        mi_score, s_score = node_clustering(emb, labels, FLAGS.clustering_algorithm)
        # Report Adjusted Mutual Information (AMI)
        mean_mutual_info.append(mi_score)
        mean_silhouette.append(s_score)


'''
# parte per salvare gli embeddings per la grid search dei parametri del node clustering
# Creazione di un DataFrame per gli embeddings
emb_df = pd.DataFrame(emb)

# Salva gli embeddings in un file CSV
emb_df.to_csv("clustering_grid_search/embeddings.csv", index=False)

# Salva le etichette in un file CSV
labels_df = pd.DataFrame(labels)
labels_df.to_csv("lustering_grid_search/labels.csv", index=False)
'''
...
if FLAGS.task == 'link_prediction':
    ...
    print("Mean Silhouette scores: ", np.mean(mean_silhouette),
          "\nStd of Silhouette scores: ", np.std(mean_silhouette), "\n \n")

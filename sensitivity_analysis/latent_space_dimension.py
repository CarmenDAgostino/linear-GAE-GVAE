...
# codice che effettua l'analisi di sensibilità sul parametro dimension,
# ovvero sulla dimensione dello spazio latente

# variabili
output_file = "results/sa_latent_dim_results.txt"
...
def write_output(output,task,dim) :
    ...
    if task == "link_prediction" :
        ...
        file.write(f"Latent dimension: {dim}\n")
...
# training per diversi valori di dimension
for task in tasks:
    ...
    for dataset in datasets:
        ...
        for model in models:
            ...
            for dim in latent_dims:
                ...
                print(f"Training and testing model {model} on dataset {dataset} and on task {task} with latent dimension {dim}...")

                modified_command = f"{train_command} --model={model} --dataset={dataset} --task={task} --dimension={dim} --kcore=True --features=True"
                output = subprocess.check_output(modified_command, shell=True) 
                output = output.decode()  

                print(f"Model trained and tested for latent dimension {dim}.")
                
                write_output(output,task,dim)

file.close()    

print("Sensitivity analysis on latent space dimension completed. Results saved in", output_file)
